...
angular
  ...
  .controller("HomeCtrl", function($scope, DataService, $filter, $http, $sce) {
    ...
    $scope.tipo_colors = function(d){
      var c = [];
       c["Espacio publico"] = "#A7C64D";
       c["Escuelas"] = "#7874B2";
       c["Salud"] = "#F1AD3C";
       c["Arquitectura"] = "#38A0D7";
       c["Hidraulica e infraestructura"] = "#BD3E93";
       c["Hidraulica"] = "#0074e0";
       c["Transporte"] = "#E84D00";
       c["Vivienda"] = "#22B496";
       c["Vialidad"] = "#22b408";
  ...
  $scope.selectTypes.push({
    ...
    DataService.getAll().then(function(data) {
      ...
      for (var l=0; l<$scope.obras.length; l++) {
        ...
        if ($scope.obras[l].destacada=="SI" && $scope.obras[l].fotos.length > 0) {
	  $scope.destacadas.push($scope.obras[l]);
        }
      }
    ...
    function renderMapGroup() {
      ...
      d3.json("geo/geometry.geojson", function (data) {
		...
		return d.properties && d.properties.dep_nombre ? d.properties.dep_nombre : '';
            });
    ...
    function prepareNodesMapGroup() {
      ...
      bubbles.nodes = $scope.obras
        ...
        .map(function(d) {
          var i = "i" + d.id,
            r = $scope.isSmallDevice ? 1.25 : 1.00,
          ...
          bubbles.clusterPoints[i] = {
            x: point[0],
            y: point[1],
            radius: $scope.isSmallDevice ? 1.25 : 2
          };
    ...
    function clickedMap(d) {
      ...
      var bounds = chart.mapPath.bounds(d),
        dx = Math.max(bounds[1][0] - bounds[0][0],200),
        dy = Math.max(bounds[1][1] - bounds[0][1],200),
      ...
      if (!$scope.isSmallDevice) {
        bubbles.circles
          ...
          .on("mouseenter", function(d) {
            ...
            //if (enModoZoom>0||$scope.selectedGroup!=="mapa") {
              $scope.tooltip
                .style("width", "300px")
       	        .transition()
                .duration(200)
